...
func BuildCommand(cfg Config) []string {
	...
	if cfg.PlaylistStart > 0 {
		args = append(args, "--playlist-start", strconv.Itoa(cfg.PlaylistStart))
	}
	if cfg.PlaylistEnd > 0 {
		args = append(args, "--playlist-end", strconv.Itoa(cfg.PlaylistEnd))
	}
	if cfg.MaxDownloads > 0 {
		args = append(args, "--max-downloads", strconv.Itoa(cfg.MaxDownloads))
	}
	if strings.TrimSpace(cfg.DownloadArchive) != "" {
		args = append(args, "--download-archive", strings.TrimSpace(cfg.DownloadArchive))
	}
	if cfg.NoPlaylist {
	...
	if cfg.AudioOnly {
		args = append(args, "-x")
		if strings.TrimSpace(cfg.AudioFormat) != "" {
			args = append(args, "--audio-format", strings.TrimSpace(cfg.AudioFormat))
		}
	}
	if cfg.OutputPath != "" {
	...
	if strings.TrimSpace(cfg.SubLangs) != "" {
		args = append(args, "--sub-langs", strings.TrimSpace(cfg.SubLangs))
	}
	...
	if strings.TrimSpace(cfg.LoadInfoJSON) != "" {
		args = append(args, "--load-info-json", strings.TrimSpace(cfg.LoadInfoJSON))
	}
	if cfg.WindowsFilenames {
		args = append(args, "--windows-filenames")
	}
	if cfg.NoMtime {
		args = append(args, "--no-mtime")
	}
	if cfg.Retries > 0 {
		args = append(args, "--retries", strconv.Itoa(cfg.Retries))
	}
	if cfg.FragmentRetries >= 0 {
		args = append(args, "--fragment-retries", strconv.Itoa(cfg.FragmentRetries))
	}
	if cfg.ConcurrentFragments > 0 {
		args = append(args, "--concurrent-fragments", strconv.Itoa(cfg.ConcurrentFragments))
	}
	if cfg.SocketTimeout > 0 {
		args = append(args, "--socket-timeout", strconv.Itoa(cfg.SocketTimeout))
	}
	if cfg.NoWarnings {
	...
	if cfg.AbortOnError {
		args = append(args, "--abort-on-error")
	}
	if cfg.IgnoreErrors {
